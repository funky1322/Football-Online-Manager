...
export const fetchMyTeam = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      const response = await axiosInstance.get("/api/team/my-team");
...
export const listPlayer = createAsyncThunk(
  ...
  async ({ playerId, askingPrice }, { rejectWithValue }) => {
    try {
      const response = await axiosInstance.post("/api/transfer/list-player", {
        playerId,
        askingPrice,
      });
...
export const unlistPlayer = createAsyncThunk(
  ...
  async ({ playerId }, { rejectWithValue }) => {
    try {
      const response = await axiosInstance.post("/api/transfer/unlist-player", {
        playerId,
      });
      return response.data.player;
    } catch (error) {
